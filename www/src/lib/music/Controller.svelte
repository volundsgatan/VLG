...
<div class="flex items-center rounded-lg bg-stone-600 p-3">
	...
	<div class="flex flex-1 justify-center text-white">
		<div class="flex items-center space-x-4 text-2xl ">
			<span
				on:click={previous}
				class="inline-flex h-12 w-12 cursor-pointer items-center justify-around rounded-full transition-all delay-100 ease-in-out hover:bg-stone-300"
				>👈</span
			>
			<span
				on:click={playpause}
				class="inline-flex h-12 w-12 cursor-pointer items-center justify-around rounded-full bg-stone-200 transition-all delay-100 ease-in-out hover:bg-stone-400"
			>
				{zone.coordinator.state.playbackState === 'PLAYING' ? '⏸️' : '▶️'}
			</span>
			<span
				on:click={next}
				class="inline-flex h-12 w-12 cursor-pointer items-center justify-around rounded-full transition-all delay-100 ease-in-out hover:bg-stone-300"
