...
export const getGuidePossibleRanges = (guide: number[], cells: Cell[]): GuideRange[] => {
	...
	for (let it = 0; it < 100; it++) {
		...
		for (const [idx, guide] of combined.entries()) {
			if (combined[idx - 1]) {
				const s = max(combined[idx - 1].start + combined[idx - 1].guideVal + 1, guide.start);
				if (s !== guide.start) {
					changed = true;
				}
		...
		for (const [idx, range] of combined.entries()) {
			const s = moveStart(range.start, idx, range.guideVal, guide, cells);
...
export const groupCanBeOfSize = (
	...
	const key = `${start}-${end}-${cells.map((c) =>
		c.state === true ? 't' : c.state === false ? 'f' : 'u'
	)}-${expectedSize}`;
...
export function* solve(
	...
	while (Q.length > 0) {
		...
		const res = logicSolver(thisState, guideRows, guideCols, maxIterations, highlightChanges);
