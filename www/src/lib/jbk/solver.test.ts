...
describe('solver', () => {
    ...
    */
	...
	for (const p of puzzles) {
		test('canSolve/' + p, () => {
			assertCanSolve(p, false);
		});

		const bruteForcePuzzles = ['ubuntu'];

		for (const p of bruteForcePuzzles) {
			test('canSolveBruteForce/' + p, () => {
				assertCanSolve(p, true);
			});
		}
	}
});

const assertCanSolve = (p: string, bruteForce: boolean) => {
	...
	const gen = solve(guide?.rows, guide?.cols, 1000, false, bruteForce);
